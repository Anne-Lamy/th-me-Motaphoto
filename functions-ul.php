...
function load_random_image_callback() {
    ...
    $first_img = new WP_Query($args);

    // Vérifie si la requête a des résultats
    if ($first_img->have_posts()) {
        while ($first_img->have_posts()) {
            $first_img->the_post();
...
function custom_api_get_photos_callback() {
    ...
    // Ajoutez les lignes d'écho ici pour imprimer les valeurs des filtres
    echo "Valeur de category: " . $category_id . "<br>";
    echo "Valeur de format: " . $format_id . "<br>";
    echo "Valeur de date: " . $date_id . "<br>";
    ...
    if ($posts->have_posts()) {
        while ($posts->have_posts()) : $posts->the_post();
            ...
            $posts_data[] = (object)array(
                'id'               => $id,
                'slug'             => get_post_field('post_name', $id), // Récupère le slug
                'type'             => 'photos',
                'title'            => get_the_title($id), // Récupère le titre
...
function full_image_lightbox() {
    ...
    // Effectue la requête WP_Query avec les arguments définis
    $query_lightbox = new WP_Query($args);

    $images = array();

    // Vérifie si la requête a des résultats
    if ($query_lightbox->have_posts()) {
        while ($query_lightbox->have_posts()) {
            $query_lightbox->the_post();
