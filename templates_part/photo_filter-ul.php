<div class="page framed-layout">
    ...
    <div class="singles">
        <div action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" id="ajax_call">   
            <div class="content-interaction ">
                <div class="single-left-bottom mini">
                    <div class="custom-select">
                        <ul name="category" id="category" class="select-post">
                            <li value=""  data-slug="" class="hidden-option">CATEGORIES
                                <ul>
                                    <?php 
                                    ...
                                    if ($categories) {
                                        foreach ($categories as $category) {
                                            echo '<li class="select-selected" value="' . $category->term_id . '" data-slug="' . $category->slug . '">' . $category->name . '</li>';
                                        }
                                    }
                                    ?>
                                </ul>
                    ...
                    <div class="custom-select">
                    <ul name="format" id="format" class="select-post">
                        <li value=""  data-slug="" class="hidden-option">FORMATS
                            <ul>
                                <?php 
                                ...
                                if ($formats) {
                                    foreach ($formats as $format) {
                                        echo '<li class="select-selected" value="' . $format->term_id . '" data-slug="' . $format->slug . '">' . $format->name . '</li>';
                                    }
                                }
                                ?>
                            </ul>
                ...
                <div class="single-right-bottom">
                    <div class="custom-select">
                    <ul type="date" name="date" id="date" class="select-post">
                        <li value=""  data-slug="" class="hidden-option">TRIER PAR
                            <ul>
                                <?php
                                ...
                                if ($query->have_posts()) { // Si on récupère des résultats ...
                                    ...
                                    while ($query->have_posts()) { // Parcourt chaque post.
                                        ...
                                        if (!in_array($post_date, $dates)) { // Vérifier si l'année n'est pas déjà présente dans le tableau.
                                            ...
                                            echo '<li class="select-selected" value="' . $post_date . '" data-slug="' . $post_date . '">' . $post_date . '</li>';
                                        }
                                    }
